...
data "terraform_remote_state" "pca" {
  ...
  config = {
    ...
    profile        = "cool-terraform-backend"
    region         = "us-east-1"
    key            = "cool-accounts-pca/terraform.tfstate"
  }

  # Note that this workspace is different from the others.  Since we use
  # data from this remote state to determine if the PCA account is in staging
  # or production (local.workspace_type), we cannot use that local variable
  # here because it would result in a Terraform "cycle" error.
  # Instead, we rely on the name of our current Terraform workspace; it must
  # match the name of one of the workspaces in cool-accounts-pca
